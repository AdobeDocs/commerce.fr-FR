...
description: Découvrez comment migrer vers Adobe Commerce as a Cloud Service.
...
Adobe Commerce as a Cloud Service fournit la plupart des configurations prêtes à l’emploi. Cependant, si vous migrez depuis une instance Adobe Commerce on Cloud ou on-premise existante, vous devez effectuer différentes actions de migration en fonction de votre configuration spécifique.

## Chemins de migration

Adobe Commerce as a Cloud Service prend en charge plusieurs chemins de migration, selon votre chronologie, votre storefront et vos personnalisations.

Au lieu d’une migration complète, Adobe Commerce as a Cloud Service prend en charge une migration par phases, en utilisant Commerce Optimizer ou une approche incrémentielle.

* **Migration incrémentielle** : cette approche implique la migration de vos données, de vos personnalisations et de vos intégrations par étapes. Cette approche est idéale pour les grands commerçants avec de nombreuses personnalisations qui souhaitent transférer progressivement leurs personnalisations et données complexes vers Adobe Commerce as a Cloud Service à leur propre rythme.

![migration incrémentielle](./assets/incremental.png){width="600" zoomable="yes"}

* **Commerce Optimizer**—Cette approche vous permet de migrer de manière itérative, en utilisant Commerce Optimizer comme phase de transition pour déplacer des personnalisations et des données complexes vers Adobe Commerce as a Cloud Service à votre propre rythme. Commerce Optimizer permet d’accéder aux services de marchandisage optimisés par les politiques et canaux de catalogue, au storefront Commerce optimisé par Edge Delivery et aux visuels de produit optimisés par AEM Assets.

![ migration itérative ](./assets/optimizer.png){width="600" zoomable="yes"}

* **Migration complète** : cette approche implique la migration de toutes les données, personnalisations et intégrations en une seule fois. Cette approche est idéale pour les commerçants plus petits avec peu de personnalisations qui souhaitent effectuer une transition rapide vers Adobe Commerce as a Cloud Service.
...
* **Migration des données** : utilisation de l’outil de migration fourni pour migrer les données de votre instance existante vers Adobe Commerce as a Cloud Service.
...
* **Intégrations** : toutes les intégrations doivent utiliser le [kit de démarrage de l’intégration](https://developer.adobe.com/commerce/extensibility/starter-kit/integration/) ou l’[API REST Adobe Commerce as a Cloud Service](https://developer.adobe.com/commerce/services/reference/cloud-service/core-admin/).
...
* **Extensions** : toutes les extensions en cours de traitement doivent être recréées en tant qu’extensions hors processus. D’ici la fin de l’année 2025, Adobe fournira 100 de nos extensions les plus populaires préchargées et prêtes à l’emploi afin de réduire les temps de création.

## Phases de migration

La migration de votre instance Adobe Commerce actuelle vers une nouvelle instance Adobe Commerce as a Cloud Service implique principalement les phases suivantes :
...
1. Commencez par examiner l’architecture, le framework d’extensibilité et les fonctionnalités de storefront d’Adobe Commerce as a Cloud Service :
...
1. Passez en revue vos intégrations tierces actuelles et confirmez la compatibilité des API avec la plateforme Adobe Commerce as a Cloud Service.

### Phase de mise en œuvre

Les étapes suivantes décrivent le processus de développement et d’exécution de la migration :

1. Créez une nouvelle instance Adobe Commerce as a Cloud Service dans le [Commerce Cloud Manager](./getting-started.md#create-an-instance).

1. Installez les applications et les personnalisations requises. Adobe Commerce as a Cloud Service est préchargé avec 100 des applications les plus populaires. Si vous avez besoin d’applications ou de personnalisations supplémentaires, vous pouvez les implémenter à nouveau à l’aide d’App Builder.
...
Avant de lancer, valider et tester votre nouvelle instance Adobe Commerce as a Cloud Service :
...
Une fois que vous avez validé et testé votre nouvelle instance de sandbox Adobe Commerce as a Cloud Service, vous pouvez lancer votre instance de production :
