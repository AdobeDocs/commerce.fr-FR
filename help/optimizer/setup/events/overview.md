...
Adobe AI agrège les données comportementales et de catalogue, créant des recommandations pour chaque type de recommandation. Le service Recommendations déploie ensuite ces recommandations sur votre storefront sous la forme d’un widget qui contient les _éléments_ de produit recommandés.
...
Le problème du _démarrage à froid_ fait référence au temps nécessaire pour qu’un modèle s’entraîne et devienne efficace. Pour les recommandations, cela signifie qu’il faut attendre qu’Adobe AI collecte suffisamment de données pour entraîner ses modèles de machine learning avant de déployer des unités de recommandation sur votre site. Plus les modèles contiennent de données, plus les recommandations sont précises et utiles. Étant donné que la collecte de données se produit sur un site en ligne, il est préférable de démarrer ce processus rapidement.
...
Les événements suivants ne sont pas spécifiques à Recommendations, mais sont nécessaires pour qu’Adobe AI interprète correctement les données de l’acheteur :
