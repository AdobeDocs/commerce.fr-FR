...
![Nouveau](../assets/new.svg) La requête `categoryTree](https://developer-stage.adobe.com/commerce/services/graphql-api/merchandising-api/index.html#query-categoryTree)` comporte désormais le paramètre d’entrée family comme facultatif. Cela permet une récupération de catégorie plus flexible en autorisant l’accès via slug sans dépendance sur un paramètre de famille spécifique. Cette requête est disponible uniquement pour les [services de marchandisage &#x200B;](https://developer.adobe.com/commerce/services/optimizer/merchandising-services/categories-storefront-implementation/).

### version 1.48

_19 février 2026_

![Nouveau](../assets/new.svg) La requête `categoryTree` dans l’API GraphQL renvoie désormais des descriptions de catégorie, des images et des balises meta d’optimisation pour les moteurs de recherche. Cette mise à jour fournit les données dont les développeurs storefront ont besoin pour afficher des images de catégories et améliorer l’optimisation du moteur de recherche avec des méta-titres, des descriptions et des mots-clés appropriés. Pris en charge uniquement sur les implémentations Commerce utilisant le [modèle de données de catalogue composable](https://developer.adobe.com/commerce/services/optimizer/) pour les storefronts découplés&lt;<!--DATA-6933-->
...
- Pour **Adobe Commerce sur l’infrastructure cloud** ou **sur site**, veillez à ce que le programme d’installation soit aligné sur le métapaquet [Service de catalogue](#catalog-service-metapackage) : utilisez le compositeur pour effectuer la mise à niveau `magento/catalog-service-installer` chaque fois que vous effectuez une mise à niveau `magento/catalog-service` ou lorsque ces notes de mise à jour décrivent une modification dont vous avez besoin, par exemple la prise en charge d’une nouvelle version de PHP. Ainsi, vos outils d’installation restent compatibles avec la version du service de catalogue que vous exécutez.
...
- Pour les projets déployés sur **Adobe Commerce sur le cloud, sur site ou Adobe Commerce as a Cloud Service, consultez la documentation suivante :
   ...
   - [Guide de l’administrateur &#x200B;](https://experienceleague.adobe.com/en/docs/commerce-admin/)
