...
| Champ | Description |
|---|---|
| `channel` | Contient des informations sur la source des données. `_id` et `_type` contiennent tous deux des [valeurs d’espace de noms](https://experienceleague.adobe.com/docs/experience-platform/xdm/schema/namespaces.html?lang=fr). |
...
Les événements de recherche fournissent des données pertinentes par rapport à l’intention de l’acheteur. Les informations sur l’intention d’un acheteur aident les commerçants à voir comment les acheteurs recherchent des articles, ce sur quoi ils cliquent et, finalement, achètent ou abandonnent. Voici un exemple d’utilisation de ces données si vous souhaitez cibler les acheteurs existants qui recherchent votre meilleur produit, mais ne l’achètent jamais. Vous devez installer l’extension [[!DNL Live Search]](../live-search/install.md) pour accéder à ces événements.
