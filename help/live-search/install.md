...
description: Découvrez la configuration requise et les étapes d’installation pour [!DNL Live Search] sur Adobe Commerce.
...
- Depuis [!DNL Live Search] version 4.0.0, l’adaptateur de recherche est [obsolète](release-notes.md#live-search-400).
...
>
>Pour plus d’informations, consultez les guides [&#x200B; Préparation à la loi HIPAA sur Adobe Commerce &#x200B;](https://experienceleague.adobe.com/fr/docs/commerce-admin/start/compliance/hipaa-ready-service/overview) et [&#x200B; Opérations &#x200B;](https://experienceleague.adobe.com/fr/docs/commerce-admin/start/compliance/hipaa-ready-service/operations), qui répertorie Live Search parmi les services Commerce non conformes à la loi HIPAA.

## Présentation des workflows

Le [!DNL Live Search] d’intégration nécessite que vous :

1. [Installer](#install) l’extension [!DNL Live Search]
1. [Configurer](#configure) les clés API
1. [Synchroniser](#sync) les données de votre catalogue
1. [Vérifier](#verify) que les données du catalogue ont été exportées
1. [Configurer](#configuredata) les données
1. [Tester](#test) la connexion
1. [Valider](#capture) vérifier que les événements capturent des données
1. [Personnaliser](#customize) votre storefront

## &#x200B;1. Installation de l’extension [!DNL Live Search] {#install}
...
1. Désactivez les modules [!DNL OpenSearch] et associés, puis installez-les [!DNL Live Search]. [!DNL OpenSearch] et [!DNL Live Search] ne peuvent pas être activés tous les deux sur la même instance Commerce.
...
Après avoir vérifié les indexeurs, l’étape suivante consiste à [configurer les clés API](#configure).
...
>[!NOTE]
>
>Le paramètre *Admin* > _[!UICONTROL Stores]_> [!UICONTROL Settings] >_[!UICONTROL Configuration]_ > **[!UICONTROL Live Search]** > **[!UICONTROL Storefront Features]** > **[!UICONTROL Enable Product Listing Widgets]** contrôle uniquement les widgets de liste de produits. Il n’existe aucun paramètre *Admin* pour désactiver l’expérience de storefront [!DNL Live Search] complète (comme la fenêtre contextuelle de recherche). Utilisez les commandes du module d’interface de ligne de commande de cette procédure pour maintenir votre recherche storefront existante active pendant que vous configurez [!DNL Live Search].
...
1. Désactivez les modules storefront [!DNL Live Search] tout en gardant les `Magento_LiveSearchAdapter` activés.

   ```bash
   bin/magento module:disable Magento_LiveSearchStorefrontPopover Magento_LiveSearchProductListing
   ```

   [!DNL Elasticsearch] continue de gérer les requêtes de recherche du storefront pendant que le service [!DNL Live Search] synchronise les données de catalogue et indexe les produits en arrière-plan. Garder `Magento_LiveSearchAdapter` activé ne fait pas basculer la recherche storefront vers [!DNL Live Search] ; le module doit uniquement rester activé pour que les dépendances du moteur de recherche Commerce continuent à se résoudre correctement.

   >[!IMPORTANT]
   >
   >Gardez `Magento_LiveSearchAdapter` activé pendant cette phase, même s’il était [obsolète](release-notes.md#live-search-400) à partir de la version 4.0.0 d’[!DNL Live Search]. `Magento\Search\Model\EngineResolver` dépend de l’activation de ce module. Par conséquent, sa désactivation interrompt la recherche storefront existante avec une erreur `500`. `Magento_LiveSearchAdapter` ne peut pas non plus être désactivé lorsque `Magento_LiveSearchMetrics` est activé, car le `composer.json` du module `Magento_LiveSearchMetrics` déclare une dépendance sur `Magento_LiveSearchAdapter`. Il n’est pas nécessaire de désactiver `Magento_LiveSearchMetrics` pour ce workflow.
...
1. Activez l’extension [!DNL Live Search] et désactivez [!DNL OpenSearch] (modules Magento Elasticsearch et OpenSearch).
   ...
   >[!NOTE]
   >
   >[!DNL OpenSearch] et [!DNL Live Search] ne peuvent pas être activés tous les deux sur la même instance Commerce. La commande disable inclut la liste des modules Commerce qui prennent en charge OpenSearch. Si aucun module n’est installé sur votre instance Commerce, une erreur `module does not exist` s’affiche.
...
## &#x200B;2. Configuration des clés API {#configure}
...
Vous pouvez vérifier les données de catalogue exportées depuis Adobe Commerce dans la base de données et confirmer que la synchronisation des données avec [!DNL Live Search] a réussi à l’aide de l’espace de travail [!DNL Live Search] GraphQL.
...
- Utilisez le [terrain de jeu &#x200B;](https://experienceleague.adobe.com/fr/docs/commerce/live-search/live-search-admin/graphql) avec la requête par défaut (voir [Référence de GraphQL](https://developer.adobe.com/commerce/webapi/graphql/schema/live-search/) pour plus de détails) pour vérifier les éléments suivants :
...
## &#x200B;5. Configurer les données {#configuredata}
...
Lorsque vous installez [!DNL Live Search] version 4.0.0 ou ultérieure, le système active par défaut les widgets de liste de produits. Lorsque les widgets sont activés, un autre composant d’interface utilisateur est utilisé pour les résultats de la recherche et pour parcourir les catégories dans les pages de liste de produits. Ce composant de l’interface utilisateur effectue des appels directs à l’[API Catalog Service](https://developer.adobe.com/commerce/webapi/graphql/schema/live-search/queries/product-search), ce qui accélère les temps de réponse.
...
## &#x200B;7. Vérifier que les événements capturent des données {#capture}
...
## &#x200B;8. Personnalisation pour votre storefront {#customize}
...
| `module-live-search-product-listing` | Couvre les résultats de recherche et de navigation (PLP) et inclut les zones de facettisation, de tri et de fiche produit sur la page. |
| `module-live-search-adapter` | **(Depuis la version 4.0.0 de [!DNL Live Search], l’adaptateur de recherche est [obsolète](release-notes.md#live-search-400).)** Achemine les requêtes de recherche du storefront vers le service [!DNL Live Search] et effectue le rendu des résultats dans le storefront. <br />- Navigation par catégorie - Achemine les requêtes du storefront [navigation supérieure](https://experienceleague.adobe.com/fr/docs/commerce-admin/catalog/catalog/navigation/navigation-top) vers le service de recherche.<br />- Recherche globale - Achemine les requêtes du champ [recherche rapide](https://experienceleague.adobe.com/fr/docs/commerce-admin/catalog/catalog/search/search) vers le service de [!DNL Live Search]. Le champ de recherche rapide se trouve dans le coin supérieur droit de la page du storefront. |
...
- Pour améliorer les performances et réduire la charge du processeur, appelez [!DNL Live Search], [!DNL Catalog Service] et [!DNL Product Recommendations] à partir d’un point d’entrée unique à l’aide d’une fédération [!DNL Catalog Service].

Pour certains cas d’utilisation, il peut être préférable d’appeler le [!DNL Catalog Service] pour obtenir des détails sur le produit et des scénarios similaires. Voir la requête [refineProduct](https://developer.adobe.com/commerce/webapi/graphql/schema/catalog-service/queries/refine-product) GraphQL pour plus d’informations.
...
- [champ [!DNL Live Search]](https://github.com/adobe/storefront-search-as-you-type)

La collecte automatique des données d’interaction utilisateur ne fonctionne pas par défaut si vous n’utilisez pas les composants standard tels que l’adaptateur de recherche, les widgets Luma ou les widgets AEM CIF. Adobe AI utilise ces données collectées pour le marchandisage intelligent et le suivi des performances. Pour résoudre ce problème, vous devez développer une solution personnalisée pour implémenter cette collecte de données en mode découplé.
...
Les développeurs et développeuses qui ont accès au code peuvent personnaliser son fonctionnement et son aspect. Ils hébergent le code sur leurs propres serveurs, mais utilisent toujours le service [!DNL Live Search].
...
Vous pouvez éventuellement installer le module `module-data-services-graphql` qui étend la couverture GraphQL existante de l’application pour inclure les champs requis pour la collecte de données comportementales en storefront.
...
[!DNL Live Search] fonctionne avec PWA Studio, mais les développeurs storefront observent des différences par rapport aux autres implémentations de Commerce, en particulier dans certains scénarios pilotés par GraphQL et certaines caractéristiques de réponse.
