---
user-guide-title: '[!DNL App Management]'
user-guide-description: Associez, configurez et gérez les applications App Builder avec votre instance Adobe Commerce.
feature: App Builder, Extensibility, Integration
source-git-commit: ab635fecb7b82294bd4a4fd045ed71931e9d265d
workflow-type: tm+mt
source-wordcount: '27'
ht-degree: 7%

---


# [!DNL App Management] {#app-management}

- [Introduction](overview.md)
- [Installer](install.md)
- Gestion de l’application {#manage-app}
   - [Gestion de l’application](manage-app.md)
   - [Dépannage](troubleshooting.md)
- [Retour à la page de départ Services Commerce](https://experienceleague.adobe.com/en/docs/commerce/user-guides/home)
