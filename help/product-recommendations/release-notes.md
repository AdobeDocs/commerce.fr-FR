...
Les notes de mise à jour décrivent les mises à jour apportées aux modules [!DNL Product Recommendations] suivants :
...
[En savoir plus sur l’assistance produit](https://experienceleague.adobe.com/en/docs/commerce-operations/release/product-availability).
...
![Correctif](../assets/fix.svg) (**[!DNL Adobe Commerce as a Cloud Service]** uniquement) Mise à jour du [panneau de prévisualisation du produit](create.md#preview-recommendations) : il s’affiche désormais uniquement pour les types de recommandation qui ont besoin d’un SKU pour la prévisualisation (par exemple, les recommandations de vente croisée, de vente incitative et basées sur des similitudes). Pour les autres types, l’aperçu est masqué. Les types personnalisés tels que **Récemment consultés** et **Recommandé pour vous** affichent un message lorsque l’aperçu n’est pas pris en charge.

_19 février 2026_

![Correction](../assets/fix.svg) Correction d’un problème en raison duquel les produits affichés dans le type de recommandation _Récemment consultés_ étaient supprimés dans un ordre inattendu lorsque la limite de produits de l’unité de recommandation était atteinte. Les produits sont désormais supprimés dans l’ordre premier entré, premier sorti (FIFO) dans lequel les produits ont été affichés.

_19 novembre 2025_

![Nouveau](../assets/new.svg) Vous pouvez désormais créer jusqu’à 50 unités de recommandation actives pour chaque type de page. Auparavant, la limite était de cinq.
...
![Nouveau](../assets/new.svg) Ajout d’une dépendance au tableau de bord [Surveillance de l’état de synchronisation des flux de données](https://experienceleague.adobe.com/en/docs/commerce-admin/systems/data-transfer/data-sync/data-feed-sync-status). Ce tableau de bord vous permet d’afficher des informations en temps réel sur l’intégrité et les performances des flux d’exportation de données qui transfèrent les données de produit et de catégorie de Commerce vers des services externes tels que les recommandations de produits.
...
![Nouveau](../assets/new.svg) [!DNL Product Recommendations] prend désormais en charge les fonctionnalités [Inventory management](https://experienceleague.adobe.com/en/docs/commerce-admin/inventory/introduction) complètes dans Commerce (anciennement appelé Multi-Source Inventory, ou MSI). Pour activer la prise en charge complète, vous devez [mettre à jour](install-configure.md#update) le module de dépendance `commerce-data-export` à la version 102.2.0+.
...
![Nouveau](../assets/new.svg) Ajout de la prise en charge de la couche de données client Adobe pour normaliser la collecte de données comportementales sur les fonctionnalités et services Adobe Commerce. Pour en savoir plus, consultez le [lisez-moi](https://github.com/adobe/commerce-events/blob/main/packages/storefront-events-collector/README.md).
...
![Nouveau](../assets/new.svg) Ajout de la possibilité de [prévisualiser les résultats des recommandations](create.md) au moment de la création. Cette mise à jour nécessite la mise à jour de votre module vers la dernière version.
...
![Correctif](../assets/fix.svg) Correction de l’erreur « Impossible de récupérer les données des options du produit » qui se produisait lors de l’exportation du catalogue.
