...
Si vous exécutez une seule instance Commerce avec **plusieurs sites web** (et vues de magasin), vous aurez peut-être besoin d’un **compte marchand PayPal différent** pour certains sites web. [!DNL Payment Services] vous permet de terminer l’intégration de PayPal **à l’échelle du site web** dans l’administrateur une fois l’instance configurée et intégrée à l’étendue **globale** (par défaut). Voir [Connecter un compte PayPal différent pour un site web](connect-website-account.md) pour connaître les conditions préalables et les étapes complètes d&#39;intégration d&#39;un compte PayPal de taille site web.
...
1. **[!UICONTROL Payment Services Sandbox ID]** et **[!UICONTROL Payment Services Production ID]** sont renseignés dans [Configuration générale](#general-configuration).

1. Un compte marchand PayPal **global** est **connecté** et vous avez **terminé l’intégration PayPal** pour cette portée par défaut (les champs **[!UICONTROL PayPal Merchant ID]** et associés sont renseignés pour la portée globale comme décrit dans [Configuration générale](#general-configuration)).

   Si l’intégration globale n’est pas terminée, basculez l’étendue de la configuration sur une **[!UICONTROL Website]**, ouvrez **[!UICONTROL Payment Services]** dans **[!UICONTROL Payment Methods]** et le bouton **[!UICONTROL Connect different account]** est **désactivé** ; terminez d’abord l’installation du connecteur et l’intégration **globale** PayPal.
...
Voir [Options de paiement](payments-options.md#paypal-payment-buttons) pour plus d’informations.
...
| [!UICONTROL Payment Action] | site internet | [action de paiement](https://experienceleague.adobe.com/en/docs/commerce-admin/config/sales/payment-methods/payment-methods) pour le mode de paiement spécifié. Options : [!UICONTROL Authorize] / [!UICONTROL Authorize and Capture] |
...
1. Dans le panneau de gauche, développez **[!UICONTROL Sales]** et choisissez **[!UICONTROL Payment Methods]**.
1. Développez la section _[!UICONTROL FEATURED ADOBE PAYMENT SOLUTION]_.
1. Dans la section _[!UICONTROL Payment Services]_, développez la section_[!UICONTROL Fastlane]_ .
...
1. Pour [définir l&#39;action de paiement](production.md#set-payment-services-as-payment-method), sélectionnez **[!UICONTROL Authorize]** ou **Autoriser et capturer**.
1. Pour activer la **[!UICONTROL 3D Secure Authentication for Fastlane]** (`Off` par défaut), choisissez `When required` pour respecter la réglementation de l’UE ou `Always` pour ajouter une couche de protection supplémentaire contre la fraude.

   >[!NOTE]
   >
   > Si l’émetteur de la carte requiert une authentification 3D Secure, cette étape ne peut pas être contournée, quelle que soit la configuration [!UICONTROL Payment Services].
...
1. Dans la section _[!UICONTROL Payment Services]_, accédez à la section_[!UICONTROL Fastlane]_ .
...
Grâce à [!DNL Apple Pay], les commerçants peuvent offrir une expérience de passage en caisse sécurisée, rapide et transparente, prenant en charge jusqu’à 99 domaines par compte de commerçant. Dans **Safari** (macOS et iOS), le bouton [!DNL Apple Pay] renseigne automatiquement les informations de paiement, de contact et d’expédition à partir de l’appareil du client, à la fois au début du passage en caisse (express) et sur la page de passage en caisse finale. Dans **Chrome, Firefox ou Microsoft Edge**, [!DNL Apple Pay] est disponible à la fois pendant le **passage en caisse express** et à l’étape **passage en caisse final** ; sur le bureau, un code QR et un **iPhone** (iOS 18 ou version ultérieure) permettent à l’acheteur de terminer le paiement dans la feuille de paie Apple. Assurez-vous que **[!UICONTROL Show Apple Pay on product detail page]** ou d’autres emplacements sont activés là où vous souhaitez effectuer un passage en caisse express.

>[!IMPORTANT]
>
> Apple rejette les paiements provenant de domaines non vérifiés. Les commerçants doivent vérifier tous les domaines où des boutons Apple Pay sont utilisés. Des restrictions par pays s’appliquent. Consultez la [documentation Apple Pay](https://developer.apple.com/documentation/apple_pay_on_the_web) pour plus d’informations sur les exigences de vérification de domaine.
...
1. Dans le panneau de gauche, développez **[!UICONTROL Sales]** et choisissez **[!UICONTROL Payment Methods]**.
1. Développez la section _[!UICONTROL FEATURED ADOBE PAYMENT SOLUTION]_.
1. Dans la section _[!UICONTROL [!DNL Payment Services]]_, développez la section_[!UICONTROL Apple Pay]_ .
1. Par **[!UICONTROL Title]**, saisissez du texte (si nécessaire) pour modifier le nom du mode de paiement, comme indiqué lors du passage en caisse.
...
| [!UICONTROL Sort order] | vue magasin | Ordre de tri du mode de paiement spécifié dans la page de passage en caisse. valeur `Numeric Only` |
| [!UICONTROL Show Apple Pay on product detail page] | vue magasin | Activez ou désactivez [!DNL Apple Pay] dans la page des détails du produit. Options : `[!UICONTROL Yes]` / `[!UICONTROL No]` |
| [!UICONTROL Show Apple Pay in mini cart preview] | vue magasin | Activez ou désactivez l’[!DNL Apple Pay] dans l’aperçu du mini panier. Options : `[!UICONTROL Yes]` / `[!UICONTROL No]` |
| [!UICONTROL Show Apple Pay on cart page] | vue magasin | Activez ou désactivez l’[!DNL Apple Pay] dans la page du panier. Options : `[!UICONTROL Yes]` / `[!UICONTROL No]` |
| [!UICONTROL Debug Mode] | site internet | Activez ou désactivez le mode de débogage. Options : `[!UICONTROL Yes]` / `[!UICONTROL No]` |
...
1. Dans le panneau de gauche, développez **[!UICONTROL Sales]** et choisissez **[!UICONTROL Payment Methods]**.
1. Développez la section _[!UICONTROL FEATURED ADOBE PAYMENT SOLUTION]_.
1. Dans la section _[!UICONTROL [!DNL Payment Services]]_, développez la section_[!UICONTROL Google Pay]_ .
...
1. Pour choisir si les acheteurs verront une page distincte de **Google Pay review** après la feuille de paie Google Pay, définissez **[!UICONTROL Skip Review]** sur `Yes` ou `No`. Lorsqu’ils sont définis sur `Yes`, les flux express pris en charge affichent **méthodes d’expédition dans la feuille de paie Google** (rappels d’expédition côté client) et peuvent s’exécuter sans l’étape de révision supplémentaire. Lorsqu’ils sont définis sur `No`, les acheteurs peuvent confirmer l’expédition et les totaux sur la page de révision avant de payer.
1. Pour activer **[!UICONTROL 3D Secure authentication]** (`Off` par défaut), choisissez `Always` ou `When required`.
1. Pour activer le mode de débogage, sélectionnez `Yes` pour le **[!UICONTROL Debug Mode]** (`No` le désactive).
...
1. Pour définir la hauteur, utilisez la valeur par défaut de la hauteur définie dans **[!UICONTROL Button Style]**.
1. Pour enregistrer vos modifications, cliquez sur **[!UICONTROL Save Config]** .
1. Accédez à **[!UICONTROL System]** > **[!UICONTROL Cache Management]**, puis cliquez sur **[!UICONTROL Flush Cache]** pour actualiser tous les caches non valides.
...
| [!UICONTROL Sort order] | vue magasin | Ordre de tri du mode de paiement spécifié dans la page de passage en caisse. valeur `Numeric Only` |
| [!UICONTROL Show Google Pay on product detail page] | vue magasin | Activez ou désactivez [!DNL Google Pay] dans la page des détails du produit. Options : `[!UICONTROL Yes]` / `[!UICONTROL No]` |
| [!UICONTROL Show Google Pay in mini cart preview] | vue magasin | Activez ou désactivez l’[!DNL Google Pay] dans l’aperçu du mini panier. Options : `[!UICONTROL Yes]` / `[!UICONTROL No]` |
| [!UICONTROL Show Google Pay on cart page] | vue magasin | Activez ou désactivez l’[!DNL Google Pay] sur la page du panier. Options : `[!UICONTROL Yes]` / `[!UICONTROL No]` |
| [!UICONTROL Skip Review] | vue magasin | Lorsque ce paramètre est défini sur `[!UICONTROL Yes]`, les flux [!DNL Google Pay] express éligibles peuvent omettre la page de révision distincte après la feuille de paie ; les modes d&#39;expédition apparaissent dans la feuille de paie Google. Lorsqu’ils sont définis sur `[!UICONTROL No]`, les acheteurs accèdent à la page de révision pour confirmer l’expédition et les totaux. Options : `[!UICONTROL Yes]` / `[!UICONTROL No]` |
...
| [!UICONTROL Show buttons at start of checkout] | vue magasin | Activez ou désactivez [!DNL PayPal payment buttons] au début du flux de passage en caisse. Options : `[!UICONTROL Yes]` / `[!UICONTROL No]` |
| [!UICONTROL Sort order] | vue magasin | Ordre de tri du mode de paiement spécifié dans la page de passage en caisse. valeur `Numeric Only` |
...
| [!UICONTROL App Switch] | vue magasin | Cette option s&#39;applique uniquement aux commerçants et aux acheteurs **US**. Lorsqu&#39;ils sont activés, tous les paiements Express PayPal essaieront de passer par l&#39;application mobile PayPal lorsqu&#39;ils seront disponibles. Options : `[!UICONTROL Yes]` / `[!UICONTROL No]` |
| [!UICONTROL Contact Preference] | vue magasin | Cette option s&#39;applique uniquement aux commerçants et aux acheteurs **US**. Si la valeur est **oui**, l&#39;adresse e-mail et le numéro de téléphone de l&#39;acheteur s&#39;affichent dans la boîte de dialogue modale de paiement PayPal. Options : `[!UICONTROL Yes]` / `[!UICONTROL No]` |
...
Les modes de paiement locaux (LPM) prennent en charge les modes de paiement spécifiques à la région et locaux, tels que les virements bancaires et les solutions de paiement localisées, parallèlement aux options existantes basées sur les cartes. Les commerçants peuvent activer ou désactiver les LPM disponibles directement dans la configuration Commerce. Consultez [Options de paiement](payments-options.md#local-payment-methods) pour plus d’informations sur les méthodes disponibles.

1. Dans la barre latérale _Admin_, accédez à **[!UICONTROL Stores]** > _[!UICONTROL Settings]_>**[!UICONTROL Configuration]**.
1. Dans le panneau de gauche, développez **[!UICONTROL Sales]** et choisissez **[!UICONTROL Payment Methods]**.
1. Développez la section _[!UICONTROL FEATURED ADOBE PAYMENT SOLUTION]_.
1. Dans la section _[!UICONTROL Payment Services]_, développez la section_[!UICONTROL Local Payment Methods]_ .
1. Par **[!UICONTROL Active]**, sélectionnez `Yes` pour activer les LPM ou `No` pour les désactiver.
1. Par **[!UICONTROL Title]**, saisissez le texte à afficher comme nom du mode de paiement lors du passage en caisse. Ce titre apparaît également dans la grille des commandes client.
1. Par **[!UICONTROL Allowed Payment Methods]**, sélectionnez les modes de paiement que vous souhaitez proposer. Les méthodes disponibles dépendent de l&#39;adresse de facturation de l&#39;acheteur et de la devise de base du site Web.
1. Par **[!UICONTROL Sort Order]**, entrez une valeur numérique pour déterminer la position d&#39;affichage des LPM dans la liste des modes de paiement disponibles.
1. Pour enregistrer vos modifications, cliquez sur **[!UICONTROL Save Config]**.
1. Accédez à **[!UICONTROL System]** > **[!UICONTROL Cache Management]**, puis cliquez sur **[!UICONTROL Flush Cache]** pour actualiser tous les caches non valides.
...
| Champ | Portée | Description |
|---|---|---|
| [!UICONTROL Active] | site internet | Activez ou désactivez les modes de paiement locaux. Options : `[!UICONTROL Yes]` / `[!UICONTROL No]` |
| [!UICONTROL Title] | vue magasin | Nom du mode de paiement affiché lors du passage en caisse et dans la grille de la commande client. Options : [!UICONTROL text field] |
| [!UICONTROL Allowed Payment Methods] | site internet | Sélectionnez les LPM à proposer. Les méthodes ne sont visibles que par les clients dont l’adresse de facturation et la devise du site web correspondent aux exigences de la méthode. Options : `Bancontact` / `BLIK` / `eps` / `iDEAL` / `MyBank` / `Przelewy24` |
| [!UICONTROL Sort Order] | vue magasin | Position d&#39;affichage du groupe LPM dans la liste des modes de paiement disponibles. Ce paramètre s&#39;applique à tous les LPM en tant que groupe ; les LPM individuels ne peuvent pas être triés indépendamment. valeur `Numeric Only` |

>[!NOTE]
>
>Chaque mode de paiement local (LPM) a des exigences spécifiques en termes de pays et de devise. Un mode de paiement s&#39;affiche uniquement lorsque le pays de l&#39;adresse de facturation du client et la devise de base du site Web correspondent à ces exigences. Par exemple, Bancontact n’apparaît que pour les clients ayant une adresse de facturation belge lorsque la devise de base est l’euro.

## Éléments de ligne

Les articles de ligne envoient des informations détaillées sur les commandes à PayPal, y compris les détails des produits, les quantités et les prix. Cette fonctionnalité est activée par défaut. Voir [Lignes](line-items.md) pour plus d&#39;informations.

### Options de configuration

| Champ | Portée | Description |
|---|---|---|
| [!UICONTROL Line Items Enabled] | site internet | Activez ou désactivez l&#39;envoi d&#39;éléments de ligne et de la répartition des montants à PayPal. Désactivez ce paramètre si vous utilisez des extensions tierces qui ajoutent des frais personnalisés non pris en charge par [!DNL Payment Services]. Options : `[!UICONTROL Yes]` / `[!UICONTROL No]` |
...
Voir [Rôles utilisateur](https://experienceleague.adobe.com/en/docs/commerce-admin/systems/user-accounts/permissions-user-roles) pour savoir comment gérer les rôles.
...
Consultez [Site, magasin et portée d’affichage](https://experienceleague.adobe.com/en/docs/commerce-admin/start/setup/websites-stores-views) pour plus d’informations sur la hiérarchie des sites web, des magasins et des vues de magasin.
