...
Installez et configurez le connecteur Adobe Commerce Optimizer pour synchroniser les données de votre catalogue Adobe Commerce avec [!DNL Adobe Commerce Optimizer], puis surveillez le statut de synchronisation des données pour vous assurer que votre storefront est à jour.
...
* [Clés d’authentification](https://experienceleague.adobe.com/fr/docs/commerce-operations/installation-guide/prerequisites/authentication-keys) pour télécharger le métapaquet Connecteur Commerce à l’aide du compositeur.
...
Si l’une des extensions suivantes est installée, désinstallez-la avant d’installer le connecteur Adobe Commerce Optimizer :
...
1. **[Installez le package Connecteur Adobe Commerce Optimizer](#install-the-adobe-commerce-optimizer-connector-package)** à l’aide du compositeur pour connecter votre instance Commerce à [!DNL Adobe Commerce Optimizer].
...
## Installation du package Connecteur Adobe Commerce Optimizer {#install-the-adobe-commerce-optimizer-connector-package}
...
## Personnalisation de la configuration de l’exportation des étendues de Commerce
...
>[!IMPORTANT]
>
>La modification des paramètres d’exportation déclenche une réindexation complète, ce qui peut prendre beaucoup de temps en fonction de la taille de votre catalogue. Adobe recommande de configurer les étendues Commerce pour qu’elles soient synchronisées avec Commerce Optimizer avant d’activer l’intégration et de démarrer la synchronisation initiale des données.
...
>[!IMPORTANT]
>
>Le traitement de la synchronisation des données démarrera en arrière-plan dès que vous aurez terminé la configuration. Selon la taille de votre catalogue, le processus de synchronisation des données peut prendre de quelques minutes à plusieurs heures.
...
![Obtention des informations d’identification](assets/developer-console-project-credentials.png){width="500" zoomable="yes"}
...
Vous pouvez surveiller et vérifier que la synchronisation fonctionne à partir de la page [Statut de la synchronisation des flux de données](https://experienceleague.adobe.com/fr/docs/commerce-admin/systems/data-transfer/data-sync/data-feed-sync-status) disponible dans l’Administration.
...
1. **Configurer [!DNL Adobe Commerce Optimizer] vues de catalogue et des politiques**
   ...
   Créez des vues et des politiques de catalogue dans l’interface utilisateur de [!DNL Adobe Commerce Optimizer]. Notez que les tarifs sont créés automatiquement à partir des groupes de clients Adobe Commerce. Pour obtenir des instructions, consultez la documentation [Vues de catalogue](../optimizer/setup/catalog-view.md) et [Politiques](../optimizer/setup/policies.md) dans le Guide d’utilisation de *Commerce Optimizer*.

1. **Configuration d’un storefront Commerce sur Edge Delivery Services**

   Suivez la [documentation de configuration de Storefront](https://experienceleague.adobe.com/developer/commerce/storefront/setup/?lang=fr) pour connecter votre storefront à l’instance [!DNL Adobe Commerce Optimizer] et commencer à diffuser des expériences commerciales personnalisées.
